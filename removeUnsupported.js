...
function isSupportedRule(selector) {
    //
    if (selector.indexOf(":hover") != -1 || selector.indexOf(":focus") != -1) {
        return false;
    }
    return true;
}

module.exports = postcss.plugin("tainwind.plugin", (options = {}) => {
    return (root) => {
        root.walkRules((rule) => {
            ...
            // 替换 1/2 ---> 1_2
            rule.selector = rule.selector.replace(/(\d)\\\/(\d)/, ($0, $1, $2) => {
                return `${$1}_${$2}`;
            });
